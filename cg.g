...
############## Statements to proof #############
# Exact
{v_new1_2 in ? /\ v_new2_2 in ?}

#{b1 in [0,1] /\ b2 in [1,2] -> error in [0,0.1]}
#{b1 in [0.5,1] /\ b2 in [1.5,2] -> alpha in ? /\ v_new1 in ? /\ v_new2 in ?}
#{b1 in [0.90,1] /\ b2 in [1.90,2] -> alpha_num_2 in ? /\ alpha_den_2 in ?}
#{b1 in [0.90,1] /\ b2 in [1.90,2] -> error in ?}
#{b1 in [0.90,1] /\ b2 in [1.90,2] /\ not alpha_den_2 in [-1b-1022,1b-1022] /\ not (r_new1_2 * r_new1_2) in [-1b-1022,1b-1022] /\ not (r_new2_2 * r_new2_2) in [-1b-1022,1b-1022] -> alpha_den_2 in ?}
#{b1 in [0.90,1] /\ b2 in [1.90,2] /\ not alpha_den_2 in [-1e-324,1e-324]-> error in ?}

#{b1 in [0.98,1] /\ b2 in [1.98,2] /\
...
#not beta_num in [-1e-324,1e-324] /\
#not beta_den in [-1e-324,1e-324] /\
#|rnd(rnd(r_new1_2 * r_new1_2) + rnd(r_new2_2 * r_new2_2))| >= 1e-324 /\
#|rnd(rnd(r_new1_2 * r_new1_2) + rnd(r_new2_2 * r_new2_2))| >= 1e-324 ->
#v1_2 in ? /\ v2_2 in ? /\ r1_2 in ? /\ r2_2 in ? /\ d1_2 in ? /\ d2_2 in ? /\ q1_2 in ? /\ q2_2 in ? /\ 
#alpha_num_2 in ? /\ alpha_den_2 in ? /\ alpha_2 in ? /\
#error in ?}
